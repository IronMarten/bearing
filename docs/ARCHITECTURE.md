...
| **A member is identified by its documentation comment ID** | X14, `DEFECTS.md` §39. The display string it replaced dropped `ref`/`out`/`in`, rendered a static constructor exactly like an instance one, gave an explicit interface implementation the containing type's name, and — for every field, which has no symbol of its own — was not a signature at all. Repairing the format case by case is the curated list §5 and §30 are both about; the doc ID separates all four by construction because the compiler has to |
