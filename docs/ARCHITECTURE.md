...
| A layering pattern is a shared **dependency set**, not a shared kind signature | §3.1 says the named dependencies per kind are the finding rather than the count, so the names are also what makes two findings the same finding. Grouping on the count let four boilerplate controllers absorb the middleware that is the section's own worked example. **The collapse this decided is gone — D54, 2026-08-26 — and the key remains**, deciding `PatternGroupSize` and the order the section prints in. The decision is kept because its measurement is what later explained the deletion: keying on identities is why a group is size 1 on every real solution, and a *"repeated across many types"* qualifier had nothing to say |
